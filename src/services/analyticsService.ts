...
export type AuthedUserWithTeam = AuthUser & { teamId: string };

type GetAnalyticsParams = {
  user: AuthedUserWithTeam;
  targetYear: number;
};

export async function getAnalytics(params: GetAnalyticsParams): Promise<NextResponse> {
  const { user, targetYear } = params;
  ...
  try {
    ...
    if (user.role === 'member') {
      ...
      if (!memberAnalytics) {
        const memberRequests = yearRequests.filter(
    ...
    // Keep the same behavior as the original route: outer route will convert to internalServerError
    throw error;
  }
}
